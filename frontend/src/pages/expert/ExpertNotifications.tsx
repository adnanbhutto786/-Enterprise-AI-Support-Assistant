    ...
    <div className="space-y-6 max-w-7xl mx-auto pb-10 relative z-10 text-white">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div>
            <div className="inline-flex items-center gap-1.5 rounded-full bg-indigo-500/10 px-3 py-1 text-xs font-semibold text-indigo-400 border border-indigo-500/20 mb-3">
              ...
              <span>Real-Time Expert Alert Stream</span>
            </div>
            <h1 className="text-3xl md:text-4xl font-extrabold tracking-tight bg-gradient-to-r from-white via-indigo-200 to-indigo-100 bg-clip-text text-transparent">
              Expert Notifications Center
            </h1>
            <p className="mt-2 text-slate-400 text-sm max-w-xl leading-relaxed">
              Stay updated on newly assigned tickets, collaboration notes from employees, and status changes in real-time.
            </p>
          </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <Card className="glass-panel border-white/5 bg-slate-900/40 rounded-2xl p-4 shadow-lg">
          ...
          <p className="text-[10px] text-slate-500 mt-0.5">All received events</p>
        </Card>
        ...
        <Card className="glass-panel border-white/5 bg-slate-900/40 rounded-2xl p-4 shadow-lg">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Ticket Alerts</span>
          ...
          <p className="text-[10px] text-slate-500 mt-0.5">Assigned / Notes / Status</p>
        </Card>

        <Card className="glass-panel border-white/5 bg-slate-900/40 rounded-2xl p-4 shadow-lg">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Escalations</span>
            <AlertTriangle className="h-4 w-4 text-amber-400" />
          </div>
          <div className="text-2xl font-black text-amber-400 mt-2">
            {notifications.filter((n) => n.type === "escalation").length}
          </div>
          <p className="text-[10px] text-slate-500 mt-0.5">Low confidence AI queries</p>
        </Card>
      </div>
          ...
          <div className="flex flex-wrap items-center gap-2 w-full md:w-auto">
            ...
            <div className="flex rounded-xl bg-slate-950/70 p-1 border border-white/10">
              {[
                ...
                { label: "Tickets", val: "ticket" },
                { label: "Escalations", val: "escalation" },
        ...
        {isLoading ? (
          ...
          <Card className="glass-panel border-white/5 bg-slate-900/40 rounded-3xl p-12 text-center">
            ...
            <p className="text-xs text-slate-400 mt-1 max-w-sm mx-auto">
              No notifications matching your filter. When tickets are assigned or notes are added, they will appear here.
            </p>
          </Card>
        ) : (
          filteredNotifications.map((notif) => (
            ...
            >
              ...
              <div className="flex items-center gap-2 flex-shrink-0 self-center">
                {/* Jump to ticket if link present or ticket */}
                <Button
                  size="sm"
                  variant="ghost"
                  onClick={() => {
                    markAsRead(notif.id)
                    navigate(notif.link || "/expert/tickets")
                  }}
