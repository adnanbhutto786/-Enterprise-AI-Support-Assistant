...
export default function DashboardLayout() {
  ...
  useEffect(() => {
    ...
    if (!token || role !== "employee" || isTokenExpired()) {
      localStorage.clear()
      navigate("/login")
    } else {
      setUserEmail(email)
  ...
  const navItems = [
    ...
    { name: "OCR Upload", href: "/ocr", icon: FileText },
  ...
  const SidebarContent = () => (
    <>
      <div className="flex h-16 shrink-0 items-center px-6 border-b border-white/5 gap-3">
        <div className="p-2 rounded-lg bg-gradient-to-tr from-indigo-500 to-purple-600 shadow-md shadow-indigo-500/20">
        ...
        <header className="sticky top-0 z-50 h-14 sm:h-16 shrink-0 w-full flex items-center justify-between border-b border-black/5 dark:border-white/10 bg-white/80 dark:bg-[#06091a]/90 backdrop-blur-2xl px-2.5 sm:px-4 md:px-8 shadow-sm">
          ...
          <div className="flex items-center gap-2 sm:gap-3 min-w-0">
            ...
            <div className="hidden md:flex items-center gap-2">
              ...
              <span className="text-sm text-indigo-600 dark:text-indigo-300 font-semibold">{userEmail}</span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-1.5 sm:gap-3 shrink-0">
            ...
            {/* Employee Notifications Bell Dropdown */}
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-[calc(100vw-2rem)] max-w-sm sm:w-80 md:w-96 rounded-2xl border-black/10 dark:border-white/10 shadow-2xl bg-white/95 dark:bg-[#0d1030]/95 backdrop-blur-xl p-0 z-[100] text-foreground dark:text-white overflow-hidden">
                ...
                <div className="flex items-center justify-between p-4 border-b border-black/5 dark:border-white/10 bg-black/[0.02] dark:bg-slate-950/60">
                  <div className="flex items-center gap-2">
                    ...
                    {unreadCount > 0 && (
                      <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-indigo-500/20 text-indigo-600 dark:text-indigo-300 border border-indigo-500/30">
                        {unreadCount} new
                      </span>
                    )}
                  </div>
                    ...
                    notifications.slice(0, 6).map((notif) => (
                      <div
                        ...
                        className={`p-3.5 transition-colors cursor-pointer flex items-start gap-3 ${
                          notif.is_read === 0
                            ? "bg-indigo-500/10 dark:bg-indigo-950/30 hover:bg-indigo-500/15"
                            : "hover:bg-black/5 dark:hover:bg-white/[0.03] opacity-75 hover:opacity-100"
                        }`}
                      >
                        <div className="p-2 rounded-xl bg-black/5 dark:bg-slate-950/80 border border-black/10 dark:border-white/10 flex-shrink-0 mt-0.5">
                          {getNotifIcon(notif.type)}
                        </div>
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between gap-1">
                            <p className="text-xs font-semibold text-foreground dark:text-white truncate">{notif.title}</p>
                            {notif.is_read === 0 && (
                          ...
                          <p className="text-[10px] text-slate-400 dark:text-slate-500 mt-1">{notif.created_at}</p>
                        </div>
                      </div>
                    ))
                  )}
                ...
                {/* Footer */}
                <div className="p-2.5 border-t border-black/5 dark:border-white/10 bg-black/[0.02] dark:bg-slate-950/60 text-center">
                  <Link
                    to="/notifications"
                    className="text-xs text-indigo-600 dark:text-indigo-300 hover:underline font-medium flex items-center justify-center gap-1 py-1 transition-colors"
                  >
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <button className="flex items-center gap-2 hover:opacity-80 transition-opacity focus:outline-none">
                  <div className="h-9 w-9 rounded-xl bg-gradient-to-tr from-indigo-500 to-purple-500 flex items-center justify-center font-bold text-xs shadow-md shadow-indigo-500/10 border border-white/20 text-white">
                    {userEmail.substring(0, 2).toUpperCase()}
                  </div>
                </button>
              </DropdownMenuTrigger>
              <DropdownMenuContent align="end" className="w-56 rounded-2xl border-black/10 dark:border-white/10 shadow-xl bg-white/95 dark:bg-slate-950/95 backdrop-blur-xl p-2 z-[100]">
                <DropdownMenuLabel className="font-normal mb-1">
                  <div className="flex flex-col space-y-1">
                    <p className="text-sm font-medium leading-none text-foreground">{userEmail}</p>
                    <p className="text-xs leading-none text-muted-foreground uppercase tracking-wider mt-1">Employee Account</p>
                  </div>
                </DropdownMenuLabel>
                <DropdownMenuSeparator className="bg-black/5 dark:bg-white/10" />
                <DropdownMenuItem 
                  onClick={() => navigate("/notifications")}
                  className="text-foreground hover:bg-black/5 dark:hover:bg-white/10 cursor-pointer py-2 px-3 rounded-lg flex items-center justify-between"
                >
                  <div className="flex items-center gap-2">
                ...
                <DropdownMenuSeparator className="bg-black/5 dark:bg-white/10 sm:hidden" />
                <DropdownMenuItem 
